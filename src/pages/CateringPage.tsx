...
import { CATERING_SERVICES, UNSPLASH_IMAGES } from '../data/siteData';
import { Calendar, CheckCircle2, Utensils, Shield, ArrowRight } from 'lucide-react';

const CANONICAL = 'https://www.dechosenevents.com/catering';
const SEO_TITLE = 'Catering Services Uganda | Corporate & Event Catering — D\'E Chosen';
const SEO_DESC = "D'E Chosen provides professional catering services in Uganda, including corporate events, wedding catering, private chefs and live cooking services.";

interface CateringPageProps {
  onOpenBooking: () => void;
}

export const CateringPage: React.FC<CateringPageProps> = ({ onOpenBooking }) => {
  return (
    <>
      {/* Per-route SEO */}
        ...
        <section className="py-20 bg-[#F4F6F8]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <span className="text-xs font-bold text-[#D99B26] tracking-widest uppercase">Catering Scope</span>
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`lg:col-span-7 p-8 sm:p-10 space-y-4 ${isEven ? 'lg:order-2' : 'lg:order-1'}`}>
                      <span className="text-xs font-bold text-[#4A777A] uppercase tracking-wider">{cat.subtitle}</span>
                      <h3 className="font-serif font-bold text-2xl sm:text-3xl text-[#1B365D]">{cat.title}</h3>
                      <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">{cat.description}</p>
