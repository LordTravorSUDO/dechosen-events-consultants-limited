...
import { Calendar, CheckCircle2, Sparkles, Layers, ArrowRight } from 'lucide-react';

const CANONICAL = 'https://www.dechosenevents.com/event-management';
const SEO_TITLE = 'Event Management Uganda | Weddings & Corporate Events — D\'E Chosen';
const SEO_DESC = 'Professional event management in Uganda for weddings, Kwanjula ceremonies, corporate events, staging, logistics and vendor coordination.';

interface EventManagementPageProps {
  onOpenBooking: () => void;
}

export const EventManagementPage: React.FC<EventManagementPageProps> = ({ onOpenBooking }) => {
  const eventServices = [
    {
    ...
    <>
      ...
      <div className="font-sans">
        ...
        <section className="py-20 bg-[#F4F6F8]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <span className="text-xs font-bold text-[#D99B26] tracking-widest uppercase">Core Deliverables</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {eventServices.map((srv, idx) => (
                <div key={idx} className="bg-white rounded-xl p-8 border border-slate-200 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between">
                  <div>
                    ...
                    <h3 className="font-serif font-bold text-2xl text-[#1B365D] mb-1">{srv.title}</h3>
                    <p className="text-xs font-semibold text-[#4A777A] uppercase tracking-wider mb-3">{srv.subtitle}</p>
                    <p className="text-xs text-slate-600 leading-relaxed mb-6">{srv.desc}</p>
                    <div className="space-y-2 pt-4 border-t border-slate-100">
                      <p className="text-[11px] font-bold text-[#1B365D] uppercase tracking-wider mb-2">Key Deliverables:</p>
        ...
        <section className="py-20 bg-white border-t border-slate-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <span className="text-xs font-bold text-[#D99B26] tracking-widest uppercase">Proven Methodology</span>
              <h2 className="font-serif font-bold text-3xl sm:text-4xl text-[#1B365D] mt-1">
                Our 5-Stage Event Execution Workflow
              </h2>
              <p className="text-sm text-slate-600 mt-2">Concept → Planning → Coordination → Production → Execution</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {workflowSteps.map((ws, idx) => (
                <div key={idx} className="bg-[#F4F6F8] rounded-lg p-6 border-t-4 border-[#1B365D] hover:border-[#D99B26] transition-colors space-y-3">
                  <div className="text-3xl font-serif font-bold text-[#D99B26]">{ws.step}</div>
                  <h4 className="font-serif font-bold text-base text-[#1B365D]">{ws.name}</h4>
