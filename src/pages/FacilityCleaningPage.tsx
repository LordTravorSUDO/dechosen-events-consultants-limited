...
import { Calendar, CheckCircle2, ShieldCheck, Sparkles, Building2, Home, PartyPopper, ArrowRight } from 'lucide-react';
...
export const FacilityCleaningPage: React.FC<FacilityCleaningPageProps> = ({ onNavigate, onOpenBooking }) => {
  ...
  return (
    <div className="font-sans">
      ...
      {/* 
        IMAGE PLACEHOLDER:
        Replace this image with a professional photograph showing a uniform-clad
        D'E Chosen cleaning squad maintaining a premium corporate facility.
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-12">
            {cleaningServices.map((cs, idx) => (
              <div
                key={idx}
                className="bg-white rounded-xl overflow-hidden border border-slate-200 shadow-md hover:shadow-2xl transition-all duration-300 grid grid-cols-1 lg:grid-cols-12 gap-0 items-center"
              >
                {/* Image */}
                  ...
                  <div className="pt-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {cs.suitableFor.map((sf, sfIdx) => (
                        <span
                          key={sfIdx}
                          className="bg-[#F4F6F8] text-slate-700 text-[11px] font-semibold px-2.5 py-1 rounded border border-slate-200"
                        >
                          {sf}
                        </span>
                      ))}
